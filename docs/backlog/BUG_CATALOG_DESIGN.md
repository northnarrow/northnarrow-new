...
| 19 | BUG-024 | FS_PROTECT_EVENTS pinned-ringbuf reuse vs NON-transient producer (FS_FIM_EVENTS sibling) *(audit sweep, §22)* | Beta-blocker | **Runtime-CONFIRMED** (2026-05-30); log-flood mitigated, fs-protect structural fix pending |
...
- **Status:** **Runtime-CONFIRMED (2026-05-30).** Log-flood facet **MITIGATED** (per-entry reject WARN rate-limited via `RejectThrottle` in `agent/src/sensors/multiplexer.rs`); the structural fs-protect fix (coordinated ring-pin + `inode_*` link-pin teardown) is **PENDING** — fs-protect denial events remain LOST on restart-without-reboot until it lands.
