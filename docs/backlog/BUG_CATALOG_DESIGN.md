...
| 25 | BUG-031 | COMBAT network isolation is **IPv4-only** (`iptables` `NORTHNARROW_COMBAT`); no `ip6tables` chain → IPv6 egress flows during COMBAT → C2/exfil bypass on any dual-stack host *(fire test #2 setup, §30)* | **Beta-blocker (HIGH, security)** | **FIXED + VM-VERIFIED 2026-06-02 (block 2, with [[BUG-039]])** — `ip6tables` mirror (`combat-rules.v6`, **preserve-NDP**) across all 4 lifecycle points; v6 carve-out applied; best-effort+WARN if absent. **VM (netns): v6 data dropped (ping6 100% loss, TCP timeout) + NDP survived (neighbor REACHABLE); reconcile cleared a stale v6 chain at boot.** Exposed+fixed an nf_tables teardown-idempotency bug. Was VM-CONFIRMED 2026-05-31 |
| 26 | BUG-032 | No allowlist/exemption for legitimate activity → autonomous defense escalates to COMBAT (network isolation) on legit tooling/admin *(fire test #2 setup, §31)* | **Beta-blocker (usability/safety)** | **FIX IMPLEMENTED 2026-06-02** — (1) corroboration/progression: a single blunt COMBAT-tier signal → ENGAGED not COMBAT; COMBAT needs a 2nd distinct signal in-window; `AntiTamperDenial` (kernel denial) split out as the lone Decisive; (2) count-filter allowlist (`escalation-allow.local`). **VM-VERIFIED (2-phase) 2026-06-02** (detect-only: single mass-write→ENGAGED, +lateral→COMBAT, AntiTamperDenial→COMBAT; allowlisted python3 lateral filtered vs unlisted→COMBAT). Was VM-CONFIRMED 2026-05-31 — agent → COMBAT on the harness's own egress |
...
| 30 | BUG-036 | **eBPF struct offsets hardcoded to one kernel (6.8.x), no BTF revalidation** → on a different / upgraded customer kernel every LSM/probe reads wrong offsets; anti-tamper silently protects nothing, FIM blind — no error, no alert. The header comment promising a BTF revalidator was **false (unimplemented)** *(BUG-034 1b portability check, §35)* | **HIGH / likely beta-blocker (security, systemic)** | **(a) FIXED + VM-VERIFIED 2026-06-01** — boot-time revalidator (`anti_tamper::btf_revalidate`, hand-rolled read-only BTF parser) checks all 38 offsets vs running-kernel BTF before any attach, refuses-to-start (exit 78 → systemd `failed`) on drift; single source in `common::btf_offsets`; false header corrected. VM (6.8.0-117): 38/38 match (no false-refuse); injected drift → loud per-offset log + exit 78 + `failed`, NRestarts=0. **(b) PENDING (post-beta)** — full CO-RE/BTF-derived portability. Blast radius: [[BUG-010]]/inode_protect, fim_watch, R011, net, [[BUG-034]] |
...
**2026-06-02 — BUG-036 (boot BTF revalidation) + BUG-032 (escalation precision) VM-verified.**
BUG-036: agent boots logging `BTF offset revalidation passed (count=38)`; injected drift → loud
per-offset log + exit 78 + `systemctl` unit `failed` (`NRestarts=0`, not a respawn loop), reverted
clean. BUG-032 two-phase, detect-only: **Phase 1** (allowlist empty) — a single mass-write burst
(count=36, the same `rustc`/`npm` shape the *pre-BUG-032* binary escalated to COMBAT minutes earlier
this session) → `POSTURE TRANSITION → ENGAGED`, no would-engage; mass-write **+** lateral
(`python3`→10.0.2.x:22) → `→ COMBAT` + would-engage (corroboration); a denied `chattr -i` ioctl on a
protected log → `AntiTamperDenial → COMBAT` on one event (Decisive). **Phase 2**
(`escalation-allow.local: lateral python3 10/8 22`) — the `python3` lateral was count-filtered (stayed
ENGAGED) while the *identical* shape from an unlisted comm (`/tmp/nnpy`) → COMBAT; each lever isolated.
Test allowlist + temp files removed afterward. **Open calibration:** pure ransomware (single-vector
mass-write) tops at ENGAGED under sustained-off — assess a mass-write-specific sustained threshold (§31).
**Current VM state:** smoke binary rebuilt with BUG-036 + BUG-032, `nn-smoke-agent` restarted clean,
**DETECT-ONLY, OBSERVING**; carve-out armed; old `northnarrow-agent.service` stopped.

**2026-06-02 — BUG-031 + BUG-039 (v6 completeness, block 2) VM-verified.** The VM is dual-stack
(`enp0s3` has a global ULA + a v6 default route via `fe80::2`); my session is v4 SSH, but the
harness egress could be v6, so isolation was tested in a **network namespace** (independent
netfilter) to avoid touching the host's global v6. **(A) Preserve-NDP proof:** netns + veth +
ULAs (`fd00::1`/`fd00::2`); applied `combat-rules.v6` in the netns → **v6 DATA dropped** (ping6
ICMPv6-echo 100% loss; a TCP connect timed out) **AND the stack stayed alive** — `ip -6 neigh`
showed `fd00::2 … REACHABLE` (NDP resolution survived); teardown (`-F`) restored ping6 2/2 (not
bricked). **(B) Detection (detect-only agent):** 22 v6 connects to a public v6:443 →
`POSTURE TRANSITION → ENGAGED (ExfiltrationPattern)`; then 3 distinct ULA hosts on :22 →
`→ COMBAT (LateralMovement)` (v6 detected + cross-family corroboration). **(C) Lifecycle
(agent, host ip6tables):** a planted stale v6 `NORTHNARROW_COMBAT` chain → boot
`reconcile_stale_chain` **detected + tore it down** (`rules_removed=1`). C first failed and
**exposed a real teardown bug** — `run_iptables_idempotent` didn't swallow nf_tables'
`Chain '…' does not exist` (`-D` to an absent chain on the other table); fixed (`already_gone()`
...
**Fix design + core (implemented locally 2026-05-30 — shared `chainlog` core; NOT built, NOT VM-validated; writer migrations staged).** A shared `agent/src/chainlog.rs` `RotatingChainLog<P, M>` replaces the 6–7 copy-pasted chain primitives incrementally. Design points:
- **Rotate-with-seal + meta-chain.** At a per-log `size_cap_bytes`, append a typed **terminator** line (keyed `rotate` object) sealing the file, then rename it to a zero-padded **sequence archive** `<base>.NNNNNN` and open a fresh active. Only **seq-1** (the first sealed file, ex-active) roots at GENESIS; every later file's first line uses `prev_hash` = the previous file's terminator `entry_hash`. So the whole file *sequence* is one continuous chain — a deleted/forged file breaks the link (closes the genesis-per-file gap).
- **On-disk formats (PERSISTENT, versioned — see the struct docs in `chainlog.rs`).** Data line = `ChainLine<P>` (payload flattened inline + optional `fmt_ver`; ABSENT on legacy v1 lines so they still verify, `Some(2)` on new); terminator = `TerminatorLine{rotate: RotateTerminator{fmt_ver,this_seq,next_seq,record_count,bytes}, …}`; manifest = `ManifestLine{manifest: ManifestEntry{fmt_ver,ts,event}, …}` with `event ∈ {Rotated{seq,terminator_hash,bytes,records}, Evicted{seq,terminator_hash}}`. Same pre-image rule as the legacy `audit::compute_entry_hash`.
- **Retention = the bound.** Keep `max_archives` sealed files; evict the oldest beyond, recording a signed `Evicted` **manifest** row. Total budget ≈ `(max_archives+1) × size_cap` per log. Honest tradeoff: a hard cap MUST eventually drop the oldest evidence — the manifest makes that drop **authenticated**, so the multi-file verifier can prove "missing prefix = eviction, not tamper."
- **Multi-file verifier** (`verify_log_set`): walks archives in seq order + the active file, carrying `expected_prev` across terminator boundaries (GENESIS only at seq-1), asserting `terminator.this_seq`, flagging **sequence gaps** + **terminator mismatch**, and accepting an evicted prefix **only** if the (independently chain-verified) manifest attests each dropped seq; reports `earliest_retained_seq`.
- **Anti-tamper dance + FAIL-SAFE** (`anti_tamper/filesystem.rs`). The state logs are `PROTECTED_INODES` + `/var/lib/northnarrow` is `chattr +i`. The agent is caller-exempt from its own `inode_protect` deny, so it may rename/create/unlink these inodes — but `+i` on the dir blocks dir-entry mutation. New `chattr_immutable_remove` + an **`ImmutabilityGuard`** (RAII) lift `+i` for the rename/create/unlink and **re-assert `+i` on EVERY exit path — Ok/Err/panic** (Drop), and the idempotent re-add HEALS a dir found already-mutable. If `lift` fails, no guard → dir stays `+i` → rotation aborts. **Boot self-heal:** `prepare_state_dir` already re-applies `chattr_immutable_add` every start, so a crash mid-dance is re-asserted at restart. `StateDirProtection` implements `chainlog::ProtectionManager`; `register_active` re-registers the new active inode via a `take_map` `ProtectedInodesHandle`.
- **Unit-tested on WSL2 (real rotation, not mocked):** real tempdir + real byte cap triggers rotation; sealed-old + new verify end-to-end across the boundary (terminator hash == new file's first `prev_hash`); retention evicts + verifier reports eviction (not tamper); a tampered archived line fails; a deleted middle archive is caught (sequence gap); reopen continues the chain.
- **Staged adoption:** core + verifier + dance shipped; **netflow.jsonl + fim_drift.jsonl migrations next** (split entry → payload + envelope, byte-compatible v1 ordering + a "legacy line still verifies" test each, wire `StateDirProtection`). The uncapped **8th log `combat-audit.jsonl`** is outside `STATE_PROTECTED_FILES` → adopts `chainlog` with `NoProtection` (no dance).
- **VM-validation pending:** the `+i`/`-i` window is sub-ms AND `inode_protect` still denies non-family tamper of the EXISTING files while the dir is `-i` (burst-test); the manifest pre-creation at bootstrap (append under `+i` needs the file to exist); real fsync/disk-pressure behavior. **Residual:** the manifest itself is unrotated (grows ~1 row/rotation — slow; cap later).

**Migration landed (2026-05-30) — chainlog core + netflow/fim_drift writers + combat B-cap.** The shared core shipped as commit `b9bd960` (contract LOCKED: byte-compat gates both ways, manifest-attested evicted-prefix root, reserved-key discriminator, fail-safe + serialized chattr dance, object-safe `Arc<dyn ProtectionManager>`). The writer migration (this pass): **netflow.jsonl + fim_drift.jsonl** now wrap `RotatingChainLog<NetFlowPayload|FimDriftPayload>` — each entry split into payload + envelope with **byte-compatible v1 field ordering** so a legacy v1 line still verifies (gates `bug026_legacy_v1_{netflow,drift}_line_still_verifies`, proving byte-equality AND end-to-end verify of a really-signed v1 line); `fmt_ver` is skipped on data lines so existing on-disk chains stay valid, new lines are v2 chaining off the last v1 line. One shared `Arc<StateDirProtection>` (one dance mutex across both writers) is built in main.rs after the anti-tamper bootstrap via `take_protected_inodes_map`; per-log caps **netflow 16 MiB×16**, **fim_drift 32 MiB×8** (~256 MiB each, conservative on 76 G). **combat-audit.jsonl** got the **B-cap** (truncate-in-place — the `+i` dir blocks a rename-rotate; it's unsigned, so no chainlog — see [[BUG-028]]). **Degrade (②):** if `take_protected_inodes_map` fails (anti-tamper FS setup failed), the writers still open with `NoProtection` + rotation disabled (append-only) so FIM-drift + netflow telemetry keeps flowing — the disk-fill vector returns only until a restart recovers anti-tamper; logged at **ERROR** (anti-tamper FS protection compromised), not warn. WSL2: `cargo build --workspace` clean + chainlog 8/8, net::drain 9/9 (incl. the rewritten return-inspecting tests, which now read the persisted row + `verify_log_set`, NOT just the return), fim::drain 16/16. **VM items:** the `take_protected_inodes_map` ordering (nothing else may mutate PROTECTED_INODES via the Ebpf after the take — else switch to opening the pinned map by path; a runtime canary/admin re-registration would break it), the chattr dance + sub-ms `-i` window, fsync/disk-pressure under load. **Staged follow-ups:** `netflow_listeners.jsonl`, `fim_baseline.jsonl`, and the two canary chains remain on the legacy copy-pasted writers.
...
## 27. BUG-028 — `combat-audit.jsonl` is unsigned/unchained (forensic gap on the most critical event)

- **Severity:** HIGH (forensic integrity — the one log proving a COMBAT entry is the one log an attacker can rewrite).
- **Status:** **Discovered 2026-05-30** during the [[BUG-026]] writer migration. Disk-fill **capped (B)**; forensic upgrade **DEFERRED to its own pass.**

**Gap.** `combat-audit.jsonl` (written by `audit_combat_reconcile`, `agent/src/main.rs`) is the ONLY agent on-disk log that is plain JSONL — no `prev_hash` / `entry_hash` / `agent_sig`, no signing. It records the most security-critical event class (COMBAT entry / stale-COMBAT reconcile), so a forced-COMBAT attacker can alter or truncate the very record that proves the COMBAT happened, with zero tamper-evidence. Every other on-disk chain (audit, FIM baseline/drift, netflow, canary) is signed + hash-chained.

**Why deferred (not folded into BUG-026).** BUG-026 is disk-fill scope; this is tamper-evidence. The B-cap (truncate-in-place — the `chattr +i` dir blocks a rename-rotate) bounds the disk now. The forensic fix (A) = adopt the `chainlog` core (signed + chained + rotated) like netflow/fim_drift — but it needs an `AgentSigningKey` + `agent_id` threaded into a deep, rare, best-effort call site **on the most critical (COMBAT) path**, where signing latency or a signing failure must NOT block or slow the teardown. That deserves a focused pass, not pressure-driven inside BUG-026.
...
- **Status:** **FIXED + VM-VERIFIED 2026-06-02** (block 2, with [[BUG-039]]); build + units green, dual-stack VM-verify passed (see VM Validation Log). Originally **VM-CONFIRMED 2026-05-31** during fire-test-#2 setup.

**Gap.** `anti_tamper::network_isolate::NetworkIsolator` built the `NORTHNARROW_COMBAT` chain on **`iptables` (IPv4) only**. There was no `ip6tables` equivalent — confirmed: in COMBAT, `ip6tables -L` showed all-ACCEPT and the host's IPv6 egress (`2607:6bc0::…`) kept flowing.

**Fix (block 2 — v6 completeness on both sides, with [[BUG-039]]).** Approach **(A) `ip6tables` mirror** (beta-minimal, matches the iptables-shaped path; nftables `inet` is the cleaner post-beta end-state).
- **`combat-rules.v6` — preserve-NDP, NOT a v4 mirror.** v4 isolation keeps ARP alive because ARP is L2, outside the filter table; v6 NDP/MLD ride ICMPv6 *in* this table, so a bare DROP would brick neighbour resolution (couldn't even reach the carve-out gateway). The ruleset RETURNs the ICMPv6 control plane (NDP 133–137, MLD 130–132/143, errors+PMTUD 1–4) + `lo` before the catch-all DROP — **isolation drops DATA, not the stack**. Type-based (not blanket `fe80::/10`), so only the control plane survives, not arbitrary link-local data.
- **All four lifecycle points** covered (the v4 split-brain lesson, doubled): `engage()` builds + `ip6tables-restore`s the v6 ruleset (best-effort + WARN — during an attack, isolate v4 even if v6 isn't provisioned, never refuse the whole COMBAT); `tear_down_chain()` clears both tables (shortcut: `release()` + `reconcile()` both delegate here); `reconcile_stale_chain()` adds an independent v6 **probe** so a v6-only stale chain is detected post-crash. The v6 carve-out (`combat_allow::generate_accept_rules_v6`) finally applies the IPv6 allow-CIDRs the v4 path parsed-but-ignored.

**Teardown robustness (fixed in block 2).** VM-verify surfaced a real bug the two-table teardown exposed: `run_iptables_idempotent` swallowed the legacy "already gone" messages but NOT nf_tables' `Chain '…' does not exist` (emitted by `-D <jump>` when the target chain is absent on one table). Since `tear_down_one` now runs on BOTH tables and one is usually empty (v4 absent when only v6 is stale, or vice-versa), that error propagated and failed the whole reconcile/release. Fixed by recognizing it as "already gone" (extracted to a tested `already_gone()` classifier; "Device or resource busy" is deliberately NOT swallowed). Pre-existing fragility, only reachable once teardown spanned two tables.

**Owned limits (recorded).** (1) **NDP link-local channel:** preserve-NDP leaves an ICMPv6 NDP/MLD control channel open on the local segment during isolation — a same-segment attacker could in principle tunnel over it; a remote/routed one cannot. The v6 analogue of v4's ARP staying up at L2. (2) **Filter-table flush** → elevated to its own bug [[BUG-041]] (operator custom rules lost across a COMBAT cycle, v4+v6).
...
- **Status:** **FIXED + VM-VERIFIED 2026-06-02** — corroboration/progression (primary) + count-filter allowlist (complement); build + units green, two-phase VM-verify passed (detect-only, see VM Validation Log). Originally **VM-CONFIRMED 2026-05-31** (fire-test-#2 setup).

**Gap.** On a host also running the Claude Code automation harness, the agent escalated OBSERVING → ALERTED → **COMBAT** + network-isolated on entirely legitimate activity: `ExfiltrationPattern` on the **harness's own API egress** (`pid=1172 comm="HTTP Client"`, outbound HTTPS), `SensitiveFileAccess` on operator config reads (`/etc/sudoers`, `fim_baseline.jsonl`), `R008` on the harness binary (`claude.exe`). Root causes: (1) any *single* COMBAT-tier heuristic jumped STRAIGHT to locked COMBAT, skipping the Engaged tier the FSM exists to provide; (2) no mechanism to allowlist/exempt known-legitimate process×destination activity from escalation.

**Fix (two levers + a deferred third).**
- **Corroboration / progression (primary, architectural).** The FSM ladder (Observing→Alerted→Engaged→Combat) is now honored: a single *blunt* (`NeedsCorroboration`) COMBAT-tier heuristic — exfil / lateral / persistence / the `/tmp`-exec + mass-write arms of confirmed-intrusion — escalates only to **ENGAGED** (alert); COMBAT (auto-isolation) requires a SECOND distinct escalation signal (Engaged-tier or above) within `CORROBORATION_WINDOW` (~15 min, a calibration knob). The split-out **`AntiTamperDenial`** (kernel LSM denial = adjudicated malice) is the one *Decisive* signal that reaches COMBAT alone. `confidence()` on `TriggerType` (common); ledger in `PostureMachine` (`posture/corroboration.rs`).
- **Allowlist (complement).** process×destination tuples (`/etc/northnarrow/escalation-allow.local`; line-based, fail-secure) **count-filter** exfil/lateral: an allowlisted `(comm, dst-CIDR[, port])` connection does NOT contribute to the threshold (count-filter, NOT whole-trigger suppress). The dst-CIDR is the load-bearing half — a spoofed comm to a non-allowlisted C2 still counts. `posture/escalation_allow.rs`.
- **Graduated friction (deferred, post-beta).** Throttle / partial-carve-out tiers between alert and full isolation need enforcement primitives the response ladder lacks; the Engaged=alert / Combat=isolate split already gives a 2-step graduation.

**Owned limits (documented, not silent gaps).**
1. **Single-vector:** with the sustained-same-type knob OFF (default), a pure single-vector attack — even sustained (massive exfil, no other distinct signal) — tops at ENGAGED (alert), never auto-isolates. Deliberate precision-over-autonomous-completeness for beta; the operator acts on the alert; the knob exists if single-vector autonomy is later required.
2. **Allowlisted-destination-as-C2:** because the count-filter keys on destination, an attacker abusing an *allowlisted* destination as C2 (e.g. exfil to a GitHub repo when GitHub ranges are listed) rides under the filter for that traffic. Inherent to any destination allowlist; keep allowlisted ranges tight.

**VM-verified 2026-06-02 (detect-only, northnarrowdev 6.8.0-117), two-phase, real kernel events.** Phase 1: a single mass-write burst (count=36) → `POSTURE TRANSITION → ENGAGED` (no isolation; the very FP — `rustc`/`npm` — that the *old* code escalated to COMBAT during a build); mass-write **+** lateral → `→ COMBAT` (corroboration); a denied `chattr` ioctl → `AntiTamperDenial → COMBAT` on one event. Phase 2: with `lateral python3 10/8 22` allowlisted, `python3`'s lateral was count-filtered (stayed ENGAGED) while the *identical* shape from an unlisted comm → COMBAT — each lever's effect isolated.

**Open calibration question (mass-write / ransomware).** Pure ransomware is single-vector — sustained mass-write with no second distinct signal — so under the default `SUSTAINED_SAME_TYPE_PROMOTES=false` it tops at ENGAGED (alert), not auto-isolation. Since ransomware is a primary XDR use case, assess whether the **mass-write arm specifically** warrants the sustained knob on (or a per-type sustained threshold): sustained-off may be right for exfil/lateral but too conservative for mass-write. Default unchanged for now — tune with operator input.

Pairs with [[BUG-033]] (detect-only) for rollout. The IPv6 detection/isolation gap is [[BUG-031]] / [[BUG-039]] (block 2, lands next).
...
## 37. BUG-038 — watchdog mislabels a BUG-036 config-refusal as "tamper suspected"

**Severity:** Enhancement (LOW, post-beta — cosmetic / forensic-clarity, not functional).
**Filed:** 2026-06-01, while wiring BUG-036's refuse-to-start surfacing.

**The point.** When BUG-036's boot-time offset revalidation finds drift, the agent logs the loud per-offset reason and exits 78 (`EX_CONFIG`). The agent unit is `Restart=no`, so systemd marks it `failed` (visible) and does not loop it — verified. But the **watchdog** owns crash-respawn: it sees the agent die, respawns it (≤5×/60 s), and on the 5th death inside 60 s hits its restart ceiling (`RESTART_CEILING_MAX_ATTEMPTS`) → `BackoffOutcome::CeilingExceeded` → `log_tamper_suspected(...)` → stands down. That give-up is therefore logged as **"tamper suspected"**, which is semantically wrong for a config refusal (a kernel-offset mismatch, not an attacker repeatedly killing the agent).

**Why it's not BUG-036 and not urgent.** The authoritative reason is already in the journal — the agent logs the per-offset drift loudly on each of the ≤5 attempts before the ceiling fires; the watchdog's "tamper suspected" line is just the give-up signal an operator sees immediately below it. The behaviour is also bounded and terminal (≤5 respawns in ~1.5 s, then stand-down — not an infinite loop). So this is forensic-clarity polish, deliberately kept OUT of BUG-036 scope (which would otherwise expand into the watchdog's restart protocol).

**Fix direction.** Teach the watchdog to distinguish a config-refusal from rapid-death-by-attacker. (a) A "config-refused" marker the agent writes on exit-78 that the watchdog checks before respawning → stand down on the FIRST refusal (skips the 5-attempt dance) + log the correct reason; or (b) read exit code 78 where reachable (the watchdog can `waitid` its OWN respawned children, though not the systemd-launched first instance). (a) is cleaner and yields a correct first-refusal message. **Related:** [[BUG-036]] (the refuse-to-start this surfaces).

---

## 38. BUG-039 — COMBAT network detection is IPv6-blind (exfil/lateral gate on AF_INET); the v6 gap is on both sides

**Severity:** HIGH (security). **Scope WITH [[BUG-031]]** as the v6-completeness block (block 2 of the COMBAT-hardening work).
**Confirmed:** 2026-06-02, during the COMBAT-hardening diagnosis.

**The point.** [[BUG-031]] is that COMBAT *isolation* is IPv4-only (no ip6tables mirror). This is the matching gap on the *detection* side: the two network Combat-tier triggers gate on `*family != 2` (AF_INET) and `return false` for anything else — `exfiltration_pattern` (`triggers.rs:734`) and `lateral_movement` (`triggers.rs:694`). So an IPv6 exfil / C2 / lateral-movement pattern is **never detected** as well as never blockable. Fixing one side without the other is half a fix: detect-v6-but-can't-block, or block-v6-but-never-trigger.

**Fix direction (with BUG-031).** Symmetric on both sides: (1) the ip6tables (or nftables `inet`) isolation mirror [[BUG-031]]; (2) widen `exfiltration_pattern` / `lateral_movement` to evaluate AF_INET6 flows — and `is_rfc1918` (`triggers.rs:767`) only inspects the first two bytes of the `[u8; 16]` address, so it has no IPv6 ULA (`fc00::/7`) / link-local (`fe80::/10`) notion and needs a v6-aware private/global classifier. The net sensors already carry the v6 address (`skc_v6_*` BTF offsets) and `family`, so the data is present at the boundary; the heuristics just discard it. **Related:** [[BUG-031]] (isolation half), [[BUG-032]] (escalation precision — block 1, lands first).

---

## 41. BUG-040 — no boot-time warning for a missing v6 ruleset on an IPv6-capable host

**Severity:** Enhancement (LOW, operational). **Filed:** 2026-06-02, designing [[BUG-031]].

**The point.** [[BUG-031]]'s v6 engage leg is best-effort: a missing `combat-rules.v6` (or absent `ip6tables`) leaves IPv6 un-isolated with a loud WARN — but that WARN fires at COMBAT-engage time, *mid-incident*. The operator should learn of a v6 isolation hole BEFORE an attack, not during one.

**Fix direction.** A natural fit beside the existing `preflight()` (BUG-036, runs at boot): if `ip6tables` is present / IPv6 is enabled but `--combat-rules-v6` points at an absent file, emit a boot-time WARN (not a refuse — refusing would break v4-only or v6-disabled hosts). Surfaces the misconfig at deploy/boot. Deliberately kept OUT of block 2 (the engage-time WARN already covers the safety case). **Related:** [[BUG-031]], [[BUG-036]] (the preflight it extends).

---

## 42. BUG-041 — COMBAT engage replaces the filter table; operator firewall rules lost across a COMBAT cycle (v4 + v6)

**Severity:** MEDIUM — potential beta-blocker (a production host often carries custom firewall rules). **Filed:** 2026-06-02 (pre-existing v4; surfaced by [[BUG-031]]'s v6 symmetry).
