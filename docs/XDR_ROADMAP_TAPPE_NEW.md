...
- Tappa 6.9.7 (RAG-Local Knowledge Base) — **✅ DELIVERED 2026-05-17** (branch `tappa-6.9.7-rag-kb-plan`, P1–P7). Sovereign tantivy BM25 over pinned MITRE ATT&CK v18.1 + SigmaHQ Linux + 6.7 `kb_seed`, R3 security-token analyzer, behind the byte-stable `RagEngine::retrieve` API, canary-gated `NN_ADE_RAG_ENABLED` (default OFF), `format_rag_block` byte-frozen as the Phase-C training contract (Q4(a)). Enables Tappa 6.9.5 Phase C (RAG-trust calibration). V1.0 forward work: hybrid candle bge-small re-rank over BM25 (plan §7); GTFOBins corpus extension (plan §4.2.3, post-beta); the two-artifact Article-13 model — signed XAI chain + a hash-chained RAG retrieval log — with the RAG log a **Tappa 13** SaaS-Backend follow-on (plan §5.1, dossier §8). Tappa 6.9.5 Phase C: PROMOTED pre-beta (17 May 2026); dataset generated post-6.9.7, conforming to the frozen `format_rag_block`. (Status annotated inline — this file is the Tappa 10.5 spec, no per-tappa board.)
