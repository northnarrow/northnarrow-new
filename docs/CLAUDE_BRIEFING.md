...
#### Task 7 known issue ISSUE_001 — RESOLVED 2026-05-19

The R009-self-kill blocker that held Task 7 in PARTIAL state was
resolved on 2026-05-19 via **Option A** (test fixture sudo-installs
binaries to `/usr/local/bin/<name>-e2etest-<ts>-<pid>` before spawn,
RAII cleanup on `AgentGuard::drop`). NO production code touched.
Full reproducer, evidence table, and resolution write-up:
`docs/issues/ISSUE_001_eni_test_r009_selfkill.md`. Task 7 ENI status
promoted PARTIAL → **SHIPPED**.
